 ...
 *
 * Env vars:
 *   CHAT_MONTHLY_BUDGET  — monthly spend cap in dollars (default: 10)
 *   CHAT_DAILY_IP_LIMIT  — max AI requests per IP per day (default: 20)
 */

import { Redis } from '@upstash/redis';

// ─── Config (override via env) ───────────────────────────────────────────────
const MONTHLY_BUDGET_DOLLARS = Number(process.env.CHAT_MONTHLY_BUDGET) || 10;
const MONTHLY_BUDGET_CENTS = MONTHLY_BUDGET_DOLLARS * 100;
const DAILY_IP_LIMIT = Number(process.env.CHAT_DAILY_IP_LIMIT) || 20;
const EST_COST_PER_REQ_CENTS = 0.1; // ~$0.001 per request (Haiku 3: ~2500 in + 100 out tokens)
const FINGERPRINT_DAILY_LIMIT = 30; // catches IP rotation — same browser signature
...
export async function checkChatBudget(req) {
  ...
  if (redis) {
    ...
    if (spent >= MONTHLY_BUDGET_CENTS) {
      return { allowed: false, reason: 'budget', message: 'Chat is temporarily unavailable. Please email support@tallyconnect.app for help.' };
    }
    if (daily >= DAILY_IP_LIMIT) {
      return { allowed: false, reason: 'daily-ip', message: `You've reached the daily message limit (${DAILY_IP_LIMIT}). Come back tomorrow, or email sales@tallyconnect.app!` };
    }
